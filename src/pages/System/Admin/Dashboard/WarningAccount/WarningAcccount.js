...
function WarningAccount() {
    ...
    const fetchAllFindInactiveAccounts = async () => {
        const response = await axios.get(
            "http://localhost:3000/api/admin/dashboard-inactiveAccount",
            {
        ...
        if (response.data && response.data.EC === 0) {
            setFindInactiveAccounts(response.data.DT.users);
                            ...
                            {findInactiveAccount.map((item, index) => {
                                ...
                                return (
                                    <tr key={index}>
                                        ...
                                        <td>{formattedDate}</td>
                                        <td style={{color: 'red'}}>{item.inactiveDays} ngày</td>
                                    </tr>
                                )
                            })}
                        </>
